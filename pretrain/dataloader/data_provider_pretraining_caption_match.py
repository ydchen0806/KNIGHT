...
def find_nearest_neighbor(coordinates, coord_list):
	...
	def __getitem__(self, index):
		...
		if self.cfg.MODEL.return_dict:
			...
			try:
				...
				temp_data['location'] = temp_data['location'].apply(convert_location)
				caption = temp_data.iloc[find_nearest_neighbor([random_z, random_y, random_x], temp_data['location'])]['description']
				caption = base_name + ', ' + caption
			except:
...
if __name__ == '__main__':
	...
	elif cfg.MODEL.match_name:
		for i in range(0, 20):
			t1 = time.time()
			tmp_data1, tmp_data2, caption = iter(data).__next__()
			img_data1 = np.squeeze(tmp_data1)
			img_data2 = np.squeeze(tmp_data2)
			# gt = np.squeeze(gt)
			print(f'img_{str(i)} shape: ', img_data1.shape, img_data2.shape, 'generate caption is : ', caption)
			if cfg.MODEL.model_type == 'mala':
				img_data1 = img_data1[14:-14,106:-106,106:-106]
				img_data2 = img_data2[14:-14,106:-106,106:-106]
				gt = gt[14:-14,106:-106,106:-106]
			...
			img_data1 = show_one(img_data1)
			img_data2 = show_one(img_data2)
			# img_affs = show_one(gt)
			im_cat = np.concatenate([img_data1, img_data2], axis=1)
