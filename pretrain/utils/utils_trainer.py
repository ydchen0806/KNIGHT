    ...
    def train_w_TextEmb(self, train_dataset):
        ...
        while niter < self.max_iterations:
            ...
            with autocast():
                ...
                img_emb1, img_emb2, proj_img_emb1, proj_img_emb2, proj_text_emb = output_dict['img_emb1'],\
                                                                                output_dict['img_emb2'],\
                                                                                output_dict['proj_img_emb1'],\
                                                                                output_dict['proj_img_emb2'],\
                                                                                output_dict['proj_text_emb']

                if self.loss_type == 'only_clip': # clip loss + barlowtwins loss
                    loss_clip_diag1, acc1_1 = self.clip_loss(x=proj_img_emb1,
                                                           y=proj_text_emb)
                    loss_clip_diag2, acc1_2 = self.clip_loss(x=proj_img_emb2,
                                                           y=proj_text_emb)
                    ...
                    # if self.device == 0:
                    print(f'epoch {niter // 1000} iter {niter} loss is {loss.item()},\
                            clip loss is {loss_clip_diag1.item() + loss_clip_diag2.item()},\
                            BT loss is {cov_loss.item()}, acc1 is {acc1.item()}')
