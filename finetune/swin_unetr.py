...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.linear = nn.Linear(16 * feature_size, 2048)
        ...
        self.adapool = nn.AdaptiveAvgPool3d(1) if spatial_dims == 3 else nn.AdaptiveAvgPool2d(1)
        ...
        if args.pretrained:
            if 'swinvit' in args.pretrained_path:
                weights = torch.load(args.pretrained_path)['state_dict']
                # print(weights.keys())
                # print(self.swinViT.state_dict().keys())
                for i in weights.keys():
                    new_name = i.replace('module.','')
                    if new_name in self.swinViT.state_dict().keys() and self.swinViT.state_dict()[new_name].shape == weights[i].shape:
                        self.swinViT.state_dict()[new_name].copy_(weights[i])
                        print(f'{new_name} is loaded !')
            else:
                weights = torch.load(args.pretrained_path)
                for i in weights.keys():
                    new_name = i.replace('swinViT.','')
                    if new_name in self.swinViT.state_dict().keys() and self.swinViT.state_dict()[new_name].shape == weights[i].shape:
                        self.swinViT.state_dict()[new_name].copy_(weights[i])
                        print(f'{new_name} is loaded !')
            # print(weights.keys())
                ...
                # else:
                #     print(f'{new_name} is not loaded')
    ...
    def forward(self, x_in):
        ...
        # print(f'hidden_states_out: {len(hidden_states_out)}')
        # for k,i in enumerate(hidden_states_out):
        #     print(f'hidden_states_out[{k}]: {i.shape}')
...
if __name__ == '__main__':
    ...
    def arg_parse():
        ...
        parse.add_argument('--pretrained_path', type=str, default='/data/ydchen/VLP/Neurips23_caption/finetune/model_swinvit.pt')
        parse.add_argument('--valid_iters', type=int, default=500)
        parse.add_argument('--backbone', type=str, default='SwinUNETR')
        args = parse.parse_args()
        return args

    args = arg_parse()

    net = SwinUNETR(args, img_size = (32,128,128),in_channels=4, out_channels=2, depths=(2,4,2,2),for_pretrain=False, feature_size=48)
    # weights = torch.load('/data/ydchen/VLP/NIPS0429/Swin_barlowClip/testclip_bt_swin_139001_iterations_encoder.pth')
    # # print(weights.keys())
    # for i in weights.keys():
    #     new_name = i.replace('swinViT.','')
    #     if new_name in net.state_dict().keys() and net.state_dict()[new_name].shape == weights[i].shape:
    #         net.state_dict()[new_name].copy_(weights[i])
    #         print(f'{new_name} is loaded')
        # else:
        #     print(f'{new_name} is not in the model')

    # print(net)
    # 参数量统计
    params = sum(param.numel() for param in net.parameters())
    print(f'参数量(M): {params/10**6:.2f}')
    net = net.cuda()
